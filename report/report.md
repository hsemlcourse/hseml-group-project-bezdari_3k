...
- выбранных raw-признаков для пайплайна: `124` (`120` top features + обязательные UI-поля);
...
- чтение данных ограничено top-N признаками по `spec_feature_importances_ohe.csv` и обязательными полями формы клиента;
- one-hot importance вроде `NAME_EDUCATION_TYPE_Higher education` маппится обратно в raw-колонку
  `NAME_EDUCATION_TYPE`, чтобы категориальные поля интерфейса реально участвовали в инференсе;
...
Финальный запуск сделан на `50000` строках, `120` top признаках и обязательных UI-полях:

| Модель | Val ROC-AUC | Test ROC-AUC | Test AP | Test F1 | Test Precision | Test Recall |
|---|---:|---:|---:|---:|---:|---:|
| `baseline_logistic_regression` | 0.757 | 0.776 | 0.265 | 0.274 | 0.171 | 0.696 |
...
|---|---|---|---:|---:|---:|
| `hist_gradient_boosting` | Boosting должен лучше ловить нелинейные зависимости в tabular risk data | Ordinal encoding + manual feature engineering | **0.782** | 0.279 | 0.035 |
| `soft_voting_ensemble` | Усреднение разных моделей стабилизирует вероятности | Ensemble из линейной модели, bagging и boosting | 0.777 | **0.280** | 0.275 |
| `logistic_regression_fe` | Финансовые ratios улучшают линейную модель | One-hot + manual feature engineering | 0.777 | 0.269 | 0.273 |
| `baseline_logistic_regression` | Простая линейная модель задаёт нижнюю границу качества | One-hot без manual feature engineering | 0.776 | 0.265 | 0.274 |
| `extra_trees` | Более случайные деревья снизят variance | Ordinal encoding + manual feature engineering | 0.769 | 0.270 | 0.306 |
| `random_forest` | Bagging по деревьям поймает взаимодействия признаков | Ordinal encoding + manual feature engineering | 0.768 | 0.259 | **0.326** |
| `svd_logistic_regression` | Уменьшение размерности снизит шум sparse-признаков | One-hot + TruncatedSVD | 0.760 | 0.255 | 0.267 |
...
Финальная модель - `hist_gradient_boosting`, потому что она дала лучший `test_roc_auc = 0.7817`.
...
Для ускорения batch scoring FastAPI использует векторизованный inference: все строки CSV собираются в один DataFrame,
а `predict_proba` вызывается один раз на весь batch. Статичные таблицы в Streamlit кэшируются через `st.cache_data`.
...
Ссылка на видео работы: [Google Drive](https://drive.google.com/file/d/1KoaLxz8RIhUecRdd8KgrqlqlVH_AVn0I/view?usp=sharing).
...
Лучший результат по основной метрике показал `hist_gradient_boosting`: `test_roc_auc = 0.7817`. Для CP3 добавлены
